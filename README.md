...
After some time, when your client decides that the predicted distribution parameters are good enough (feel free to choose this moment by yourself), it should switch automatically into an Anomaly Detection stage. 

An incoming frequency is considered an anomaly, if it differs from the expected value by more than *STD* to any side (to the left or to the right, as the distribution is symmetric).

For now you should just write found anomalies into a log.

<h2 id="chapter-vi" >Chapter III</h2>
...
<h2 id="chapter-vii" >Chapter IV</h2>
